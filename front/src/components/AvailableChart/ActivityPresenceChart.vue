    ...
    <div class="activity-presence-chart__table">
      <div>
        <div class="activity-presence-chart__table-header">
          <div class="activity-presence-chart__table-header-activity-cell">
            Вид деятельности
          </div>
          <div class="activity-presence-chart__table-header-specialist-cell">
            Специалист
          </div>
          <div
            class="activity-presence-chart__table-header-day-cell"
        ...
        <div class="activity-presence-chart__table-body">
          <div
            class="activity-presence-chart__activity-row"
            v-for="activity in mainData"
            :key="activity.id"
          >
            <div class="activity-presence-chart__activity-cell">
              <div
                ...
                :style="{
                  'background-color': activity.color + '4d',
                  border: '1px solid ' + activity.color + '99',
                }"
              >
                {{ activity.name }}
              </div>
            ...
            <div class="activity-presence-chart__specialist-presence-container">
              <div
                class="activity-presence-chart__specialist-presence-row"
                v-for="specialist in activity.specialists"
                :key="specialist.id"
              >
                <div class="activity-presence-chart__specialist-cell">
                  {{ specialist.fullName }}
                </div>
                <div class="activity-presence-chart__specialist-chart-holder">
                  <div
                    ...
                    :key="interval.id"
                    :class="{
                      start: interval.hasStart,
                      end: interval.hasEnd,
                      additional: !specialist.isMainActivity,
                    }"
                    :style="{
                      width:
                        (interval.daysCount * 100) /
...
export default {
  ...
  computed: {
    mainData() {
      let result = this.activities.map((activity) => {
        return {
          id: activity.id,
      ...
      this.tableData.specialists.forEach((specialist) => {
        specialist.activities.forEach((specialistActivity) => {
          let activityItem = result.find(
            (item) => item.id == specialistActivity.activity.id
          );
          if (activityItem)
            activityItem.specialists.push({
              id: specialist.id,
              fullName: specialist.fullName,
              isMainActivity: specialistActivity.is_main,
              presence: specialist.presence,
            });
        });
      });
