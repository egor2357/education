  ...
  <a-spin :spinning="loading">
    <div class="exercise-details">
      <div class="top-bar">
        ...
        <div class="title">Выполнение упражнения</div>
      ...
      <div class="table-block" v-if="exerciseReports.length">
        <div class="exercise-name">{{ exerciseReports[0].exercise.name }}</div>
        ...
        <div class="table-holder">
          <div>
            ...
            <div class="table-body" v-if="exerciseReports.length">
              <div
                class="table-row"
                v-for="exerciseReport in exerciseReports"
                :key="exerciseReport.id"
                @click="goToJob(exerciseReport.job)"
              >
                <div class="table-cell table-cell_date">
                  {{ exerciseReport.job.date | toRuDateString }}
                </div>
                <div class="table-cell table-cell_activity">
                  <div
                    :style="{
                      'background-color': `${exerciseReport.job.activity.color}10`,
                      border: `1px solid ${exerciseReport.job.activity.color}99`,
                    }"
                    class="table-cell__activity-name"
                  >
                    {{ exerciseReport.job.activity.name }}
                  </div>
                </div>
                <div class="table-cell table-cell_specialist">
                  {{
                    exerciseReport.job.specialist
                      ? exerciseReport.job.specialist.__str__
                      : ""
                  }}
                </div>
                <div class="table-cell table-cell_job">
                  <div class="table-cell__job">
                    <div class="table-cell__job-topic">
                      {{ exerciseReport.job.topic }}
                    </div>
                  </div>
                  <div
                    class="table-cell__job-mark"
                    :style="{
                      'background-color': getColorByMark(exerciseReport.mark),
                    }"
                  ></div>
                </div>
              </div>
  ...
  methods: {
    ...
    async fetchExerciseReports() {
      try {
        ...
        let fourthQParameter = `exercise_id=${this.$route.params.id}`;
        let QParameters = `?${firstQParameter}&${secondQParameter}&${fourthQParameter}`;
        let res = await this.$axios.get(
          `/api/exercise_reports/${QParameters}&ordering=${this.activeSort}`
        );
        if (res.status === 200) {
          this.exerciseReports = res.data;
        } else {
    ...
    async clickedSort(param) {
      ...
      await this.fetchExerciseReports();
    },
  },
