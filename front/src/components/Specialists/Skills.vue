                ...
                <div class="table-row" v-for="skill in direction.skills" :key="skill.id">
                  ...
                  <div class="table-row__column_coefficient">
                    <div class="table-cell">
                      ...
                      />
                      <a-input-number class="skill-coefficient"
                        v-if="skillsData[skill.id]"
                        :step="0.1"
                        :min="0"
                        :max="1"
                        ...
                        @blur="setSkillCoefficient($event, skillsData[skill.id].linkId)"
                      />
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
      ...
      for (let area of this.areas)
      {
        for (let direction of area.development_directions)
        {
          for (let skill of direction.skills)
          {
            ...
            this.$set(this.skillsData, skill.id, {linkId: null, coefficient: null});
          }
        }
      }
    ...
    async setSkillState(event, skillId){
      ...
      {
        let res = await this.deleteSkill({linkId: this.skillsData[skillId].linkId, specialistId: this.specialistId});
        if (res.status === 204) {
          this.makeSkillsData();
